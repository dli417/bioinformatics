...
# Graph 3: Brynum bornholmense abundance over time
...
# Graph 4: all Pohlia genus abundance over time
...
long_spp_bryum_all_gg + geom_line(aes(col = abundance)) +
                        geom_point(aes(col = abundance)) +
                        ggtitle("Total abundance of Brynum genus") +
                        theme(plot.title = element_text(hjust = 0.5)) +
                        xlab("Date") +
                        ylab("Abundance") +
                        labs(col = "Abundance") +
...
head(long_spp)
test1 <- ggplot(data = long_spp, aes(x = date, y = abundance))
test1 + geom_point(aes(col = species)) + theme(legend.position = "none") + facet_wrap(. ~ species)
test1 + geom_point(aes(col = primary_threat)) + theme(legend.position = "none") + facet_wrap(. ~ primary_threat)
# test1 + geom_point(aes(col = secondary_threat))
# test1 + geom_point(aes(col = tertiary_threat))
